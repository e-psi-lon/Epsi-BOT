...
class Playlists(commands.Cog):
	...
	async def create_from_queue(self, ctx: discord.ApplicationContext, name: str, playlist_type: str):
		...
		server = await Server.get(server_id=ctx.interaction.guild.id).prefetch_related("queue", "playlists", "playlists__playlist")
	...
	async def create_from_youtube(self, ctx: discord.ApplicationContext, url: str, name: str, playlist_type: str):
		await ctx.response.defer()
		try:
			server = await Server.get(server_id=ctx.interaction.guild.id).prefetch_related("playlists", "playlists__playlist")
			asker, _ = await Asker.get_or_create(discord_id=ctx.user.id)
			user_playlists = await asker.playlists.all().prefetch_related("playlist")
	...
	async def play(self, ctx: discord.ApplicationContext, name: str):
		...
		server = await Server.get(server_id=ctx.guild.id).prefetch_related("queue", "queue__song", "playlists", "playlists__playlist")
		asker = await Asker.get(discord_id=ctx.user.id).prefetch_related("playlists", "playlists__playlist")
	...
	async def list_playlist(self, ctx: discord.ApplicationContext, playlist_type: str):
		await ctx.response.defer()
		playlists = await ((await Server.get(server_id=ctx.guild.id)).playlists.all().prefetch_related("playlist", "playlist__songs") if playlist_type == "server" else (await Asker.get(discord_id=ctx.user.id)).playlists.all().prefetch_related("playlist", "playlist__songs"))
		if not playlists:
			return await ctx.respond(
	...
	async def copy(self, ctx: discord.ApplicationContext, name: str):
		...
		if name.endswith(" - SERVER"):
			...
			return await ctx.respond(embed=EMBED_ERROR_PLAYLIST_NAME_DOESNT_EXIST
									.add_field(name="Existing server playlists:",
												value="\n- ".join([playlist.playlist.name for playlist in await (await Server.get(server_id=ctx.guild.id)).playlists.all().prefetch_related("playlist")]))
									.add_field(name="Existing user playlists:",
												value="\n- ".join([playlist.playlist.name for playlist in await (await Asker.get(discord_id=ctx.user.id)).playlists.all().prefetch_related("playlist")])))
		playlist = await Playlist.get(name=name)
		...
		if playlist in asker.playlists:
			...
			await PlaylistSong.bulk_create((await Playlist.get(name=name)).songs.all())
