from quart import Blueprint, session, redirect, url_for, render_template, request, current_app
...
async def server_detail(server_id: int):
	...
	async def _server_detail():
		...
		server_data = ConfigData(
			config.loop_song, config.loop_queue, config.random,
			config.position, await config.queue, server_id,
