...
def _embed_error(description: str) -> discord.Embed:
	...
	return discord.Embed(
		title="Error", description=description, color=discord.Color.dark_red()
	)
...
EMBED_ERROR_PLAYLIST_NAME_DOESNT_EXIST: Final[discord.Embed] = _embed_error(
	"A playlist with this name does not exist. Existing playlists:"
)
EMBED_ERROR_BOT_NOT_CONNECTED: Final[discord.Embed] = _embed_error(
	"The bot is not connected to a voice channel"
)
EMBED_ERROR_BOT_NOT_PLAYING: Final[discord.Embed] = _embed_error(
	"The bot is not playing anything."
)
EMBED_ERROR_INDEX_TOO_HIGH: Final[discord.Embed] = _embed_error(
...
EMBED_ERROR_VIDEO_TOO_LONG: Final[discord.Embed] = _embed_error(
	f"The video is too long. A maximum length is {MAX_TRACK_LENGTH // 60} minutes."
)
EMBED_ERROR_NOT_BOT_OWNER: Final[discord.Embed] = _embed_error(
	"You are not the owner of the bot."
)
EMBED_ERROR_PLAYLIST_EXISTS: Final[discord.Embed] = _embed_error(
...
GET_FILE_HTTP_URL: Final[re.Pattern] = re.compile(
	r"^https?://[^\s/$.?#]+\.[^\s/]+/.*?([^/]+\.[^/\s?#]+)(?:\?.*)?(?:#.*)?$"
)
