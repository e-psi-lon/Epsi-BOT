...
async def get_playlists(ctx: discord.AutocompleteContext) -> list[str]:
	...
	async with database_context():
		if ctx.interaction.guild is None or ctx.interaction.user is None:
			return []
		config = await Server.get(server_id=ctx.interaction.guild.id).prefetch_related(
			"playlists", "playlists__playlist"
		)
		user = await User.get(discord_id=ctx.interaction.user.id).prefetch_related(
			"playlists", "playlists__playlist"
		)
		return [
			playlist.playlist.name + " - SERVER" for playlist in config.playlists
	...
	async with database_context():
		if ctx.options["playlist"].endswith(" - SERVER"):
			...
			server = await Server.get(
				server_id=ctx.interaction.guild.id
			).prefetch_related(
				"playlists", "playlists__playlist", "playlists__playlist__songs"
			)
