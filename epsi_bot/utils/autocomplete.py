...
from .models import Asker, Server, database_context
...
async def get_playlists(ctx: discord.AutocompleteContext) -> list[str]:
	...
	async with database_context():	
		config = await Server.get(server_id=ctx.interaction.guild.id).prefetch_related("playlists", "playlists__playlist")
...
async def get_playlists_songs(ctx: discord.AutocompleteContext) -> list[str]:
	...
	async with database_context():
		if ctx.options['playlist'].endswith(" - SERVER"):
			server = await Server.get(server_id=ctx.interaction.guild.id).prefetch_related("playlists", "playlists__playlist", "playlists__playlist__songs")
			...
			user = await Asker.get(discord_id=ctx.interaction.user.id).prefetch_related("playlists", "playlists__playlist", "playlists__playlist__songs")
