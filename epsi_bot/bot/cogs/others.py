...
class Others(commands.Cog):
	...
	@commands.slash_command(
		name="download_file", description="Download the audio of a youtube video"
	)
	@discord.option(
		"file-format",
		str,
		description="The file_format of the file",
		choices=["mp3", "ogg"],
		required=False,
		default="ogg",
		parameter_name="file_format",
		min_length=3,
		max_length=3,
	)
	async def download_file(
		self, ctx: discord.ApplicationContext, query: str, file_format: str
	) -> None:
		...
		try:
			...
			try:
				...
				match file_format:
					...
					case _:
						await ctx.respond(
							embed=discord.Embed(
								title="Error",
								description="Invalid file_format.",
				...
				await ctx.respond(
					...
					file=discord.File(buffer, filename=f"{video.title}.{file_format}"),
				)
			except pytubefix.exceptions.PytubeFixError:
				await ctx.respond(
					embed=discord.Embed(
						title="Error",
						description="Error while downloading song.",
						color=discord.Color.dark_red(),
					)
				)
				return
			...
			await ctx.respond(
				embed=discord.Embed(
					title="Select audio",
					description="Select an audio to download.",
					color=discord.Color.green(),
				),
				view=view,
			)
	...
	async def lyrics(self, ctx: discord.ApplicationContext) -> None:
		...
		if not await server.queue.all():
			...
			await ctx.respond(
				embed=discord.Embed(
					title="Error",
					description="This command is only available for youtube videos.",
					color=discord.Color.dark_red(),
				)
			)
			return
		...
		if not lyrics:
			...
			await ctx.respond(
				embed=discord.Embed(
					title="Lyrics", description=lyrics, color=discord.Color.green()
				)
			)
