...
class Playlists(commands.Cog):
	...
	playlist = SlashCommandGroup(
		name="playlist", description="Commands related to playlists"
	)
	...
	@discord.option(
		...
		choices=["server", "user"],
		default="server",
		parameter_name="playlist_type",
		min_length=4,
		max_length=6,
	)
	async def create_from_queue(
		self, ctx: discord.ApplicationContext, name: str, playlist_type: str
	) -> None:
		...
		if ctx.interaction.guild is None:
			...
			return
		clean_name, parsed_type, error = validate_and_parse_playlist(
			name, playlist_type
		)
		if error:
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Playlist",
				description=f"Playlist {clean_name} created.",
				color=discord.Color.green(),
			)
		)

	@create.command(
		name="from-youtube", description="Creates a playlist from a youtube playlist"
	)
	...
	async def create_from_youtube(
		self, ctx: discord.ApplicationContext, url: str, name: str, playlist_type: str
	) -> None:
		await ctx.response.defer()
		try:
			if ctx.interaction.guild is None:
			...
			for idx, video in enumerate(playlist.videos):
				song, _ = await Song.get_or_create(
					name=video.title, url=video.watch_url
				)
				songs_to_create.append(
					PlaylistSong(
						asker=user, playlist=db_playlist, song=song, position=idx + 1
					)
				)
			...
			await ctx.respond(
				embed=discord.Embed(
					title="Error",
					description="You must use an url of a youtube playlist",
	...
	async def delete(self, ctx: discord.ApplicationContext, name: str) -> None:
		...
		if error is None and parsed_type is None:
			...
			await ctx.respond(
				embed=EMBED_ERROR_PLAYLIST_NAME_DOESNT_EXIST.add_field(
					name="Existing server playlists:",
					value="\n".join([p.playlist.name for p in server.playlists]),
				).add_field(
					name="Existing user playlists:",
		...
		if not check_playlist_exists(clean_name, parsed_type, server, user_playlists):
			playlist_names = (
				[p.playlist.name for p in server.playlists]
				if parsed_type == PlaylistType.SERVER
				else [p.playlist.name for p in user_playlists]
			)
			await ctx.respond(
				embed=EMBED_ERROR_PLAYLIST_NAME_DOESNT_EXIST.add_field(
					name=f"Existing {parsed_type.value} playlists:",
					value="\n".join(playlist_names),
				)
			)
			return
		...
		if parsed_type == PlaylistType.SERVER:
			playlist_to_del = await ServerPlaylist.filter(
				playlist__name=clean_name, server=server
			).get()
		else:
			playlist_to_del = await UserPlaylist.filter(
				playlist__name=clean_name, user=user
			).get()
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Playlist",
				description=f"Playlist {clean_name} deleted.",
				color=discord.Color.green(),
			)
		)
	...
	@discord.option(
		"query",
		str,
		description="The YouTube video to add to the playlist",
		required=True,
	)
	async def add(self, ctx: discord.ApplicationContext, name: str, query: str) -> None:
		...
		try:
			...
			await ctx.respond(
				embed=discord.Embed(
					title="Playlist",
					description=f"Song added to playlist {clean_name}.",
					color=discord.Color.green(),
				)
			)
		except IndexError:
			await ctx.respond(
				embed=discord.Embed(
					title="Error",
					description="Error while getting song.",
					color=discord.Color.dark_red(),
				)
			)
		except PytubeRegexMatchError:
	...
	async def remove(
		self, ctx: discord.ApplicationContext, name: str, song: str
	) -> None:
		await ctx.response.defer()
		...
		if song_to_remove is None:
			await ctx.respond(
				embed=discord.Embed(
					title="Error",
					description="This song is not in the playlist.",
					color=discord.Color.dark_red(),
				)
			)
			return
		playlist = await Playlist.get(name=clean_name)
		playlist_song = await PlaylistSong.get_or_none(
			playlist=playlist, song=song_to_remove
		)
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Playlist",
				description=f"Song {song_to_remove.name} removed from playlist {clean_name}.",
				color=discord.Color.green(),
			)
		)
	...
	async def play(self, ctx: discord.ApplicationContext, name: str) -> None:
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Play",
				description=f"Playing {server.queue[server.position].song.name}",
				color=discord.Color.green(),
			)
		)
	...
	async def list_playlist(
		self, ctx: discord.ApplicationContext, playlist_type: str
	) -> None:
		await ctx.response.defer()
		...
		for index, playlist in enumerate(playlists[:24]):
			...
			embed.add_field(
				name=f"__{playlist.playlist.name}__ :",
	...
	async def show(self, ctx: discord.ApplicationContext, name: str) -> None:
		...
		playlist_songs = (
			await (await Playlist.get(name=clean_name))
			.songs.all()
		...
		for index, playlist_song in enumerate(playlist_songs[:24]):
			...
			embed.add_field(
				name=f"{index + 1}.", value=f"__[{song.name}]({song.url})__"
			)
	...
	async def rename(
		...
		if not check_playlist_exists(clean_name, parsed_type, server, user_playlists):
			playlist_names = (
				server_playlists_names
				if parsed_type == PlaylistType.SERVER
				else user_playlists_names
			)
			await ctx.respond(
				embed=EMBED_ERROR_PLAYLIST_NAME_DOESNT_EXIST.add_field(
					name=f"Existing {parsed_type.value} playlists:",
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Playlist",
				description=f"Playlist {clean_name} renamed to {new_name}.",
				color=discord.Color.green(),
			)
		)

	@playlist.command(
		name="copy", description="Copies a playlist to the other playlist type"
	)
	@discord.option(
		"name",
		str,
	...
	async def copy(self, ctx: discord.ApplicationContext, name: str) -> None:
		...
		source_songs = await source_playlist.songs.all().prefetch_related(
			"song", "asker"
		)
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Playlist",
				description=f"Playlist {clean_name} copied.",
				color=discord.Color.green(),
			)
		)
...
async def get_common_data(
	...
	if server is None:
		raise ValueError(
			"Server not found in database. Bot may need to be reinitialized for this server."
		)
	...
	if playlist_type:
		...
		if not parsed_type:
			return (
				...
				discord.Embed(
					title="Error",
					description="Invalid playlist type. Use 'server' or 'user'.",
				),
			)
