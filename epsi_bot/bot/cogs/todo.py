...
class Todo(commands.Cog):
	...
	todo = SlashCommandGroup(
		name="todo",
		description="Commands related to the to-do list",
	...
	@discord.option(
		"index",
		int,
		description="The index of the line to add",
		required=False,
		default=None,
	)
	async def add_line(
		self, ctx: discord.ApplicationContext, line: str, index: int
	) -> None:
		await ctx.response.defer()
		if ctx.channel.id != 1128286383161745479:
			embed = discord.Embed(
				title="Ajout d'une ligne",
				description="Cette commande ne peut être utilisée que dans"
		...
		lines.insert(
			index - 1,
			discord.EmbedField(name=f"**`{index + 1}.`**", value=line, inline=False),
		)
		...
		await message.edit(
			embed=discord.Embed(
				title="To-Do List",
				description="Les points suivant sont les "
		...
		embed = discord.Embed(
			title="Ajout d'une ligne",
			description=f"La ligne {added_line.name} a été ajoutée au message "
			f"{message.jump_url}",
		)
		await ctx.respond(embed=embed, delete_after=30)
	...
	@discord.option(
		"index", int, description="The index of the line to remove", required=True
	)
	async def remove_line(self, ctx: discord.ApplicationContext, index: int) -> None:
		...
		if 0 < index <= len(lines):
			...
			embed = discord.Embed(
				title="Erreur",
				description="L'index fourni est hors des limites.",
				color=discord.Color.dark_red(),
			)
			await ctx.respond(embed=embed, delete_after=30)
			return
		...
		await message.edit(
			embed=discord.Embed(
				title="To-Do List",
				description="Les points suivant sont les différentes"
		...
		embed = discord.Embed(
			title="Suppression d'une ligne",
			description=f"La ligne {old_line.name} a été "
	...
	@discord.option(
		"index", int, description="The index of the line to edit", required=True
	)
	@discord.option("line", str, description="The new line", required=True)
	async def edit_line(
		self, ctx: discord.ApplicationContext, index: int, line: str
	) -> None:
		await ctx.response.defer()
		if ctx.channel.id != 1128286383161745479:
			embed = discord.Embed(
				title="Modification d'une ligne",
				description="Cette commande ne peut être utilisée "
				"que dans le channel "
				"<#1128286383161745479>",
			)
			await ctx.respond(embed=embed, delete_after=30)
		...
		if " - Assigned to <@" in lines[index - 1].value:
			lines[index - 1].value = (
				line
				+ lines[index - 1].value[
					lines[index - 1].value.find(" - Assigned to <@") :
				]
			)
		else:
		...
		embed = discord.Embed(
			title="Modification d'une ligne",
			description=f"La ligne {updated_line.name} a"
	...
	@discord.option(
		"index", int, description="The index of the line to assign", required=True
	)
	@discord.option(
		"user",
		discord.User,
	...
	async def assign(
		self, ctx: discord.ApplicationContext, index: int, user: discord.User
	) -> None:
		await ctx.response.defer()
		if ctx.channel.id != 1128286383161745479:
			embed = discord.Embed(
				title="Assignation d'une ligne",
				description="Cette commande ne peut être utilisée "
				"que dans le channel "
				"<#1128286383161745479>",
				color=discord.Color.dark_red(),
		...
		if assigned_user:
			if len(assigned_user) == 1:
				...
				new_line = " - Assigned to " + ", ".join(
					f"<@{int(user_id)}>" for user_id in assigned_user[:-1]
				)
	...
	@todo.command(
		name="tuto", description="Sends a tutorial on how to use the to-do list"
	)
	async def tuto(self, ctx: discord.ApplicationContext) -> None:
		...
		embed.add_field(
			name="Tutoriel",
			value="Pour afficher ce tutoriel, utilisez la commande `/todo tuto`",
			inline=False,
		)
