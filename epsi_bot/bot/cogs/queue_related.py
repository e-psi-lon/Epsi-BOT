...
class Queue(commands.Cog):
	...
	async def queue(self, ctx: discord.ApplicationContext) -> None:
		...
		for i, queue_elem in enumerate(server.queue):
			...
			if i == server.position:
				embed.add_field(
					name=f"{i + 1}. {song.name} - __**Now Playing**__",
					value=f"{song.url} asked by <@{queue_elem.asker.discord_id}>",
					inline=False,
				)
			else:
				embed.add_field(
					name=f"{i + 1}. {song.name}",
					value=f"{song.url} asked by <@{queue_elem.asker.discord_id}>",
					inline=False,
				)
	...
	async def skip(self, ctx: discord.ApplicationContext, by: int) -> None:
		await ctx.response.defer()
		...
		if (
			by < 0
			or by >= len(server.queue)
			or server.position + by >= len(server.queue)
		):
			server.loop_song = loop_song
			server.loop_queue = loop_queue
			await server.save()
			await ctx.respond(
				embed=discord.Embed(
					title="Error",
					description=f"Index {by} out of range.",
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Skip",
				description=f"Skipped {by} songs.",
				color=discord.Color.green(),
			)
		)

	loop = SlashCommandGroup(
		name="loop", description="Commands related to looping songs"
	)
	...
	async def loop_song(self, ctx: discord.ApplicationContext, state: bool) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id)
		if state is None:
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Loop",
				description=f"Loop song set to {'on' if state else 'off'}.",
				color=discord.Color.green(),
			)
		)
	...
	@discord.option("state", bool, description="The loop state", required=False)
	async def loop_queue(self, ctx: discord.ApplicationContext, state: bool) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id)
		if state is None:
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Loop",
				description=f"Loop queue set to {'on' if state else 'off'}.",
				color=discord.Color.green(),
			)
		)

	@commands.slash_command(name="now", description="Shows the current song")
	async def now(self, ctx: discord.ApplicationContext) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id).prefetch_related(
			"queue", "queue__song", "queue__asker"
		)
		if not server.queue:
			await ctx.respond(embed=EMBED_ERROR_QUEUE_EMPTY)
			return
	...
	remove = SlashCommandGroup(
		name="remove", description="Commands related to removing songs from the queue"
	)
	...
	async def remove_name(self, ctx: discord.ApplicationContext, song: str) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id)
		if not await server.queue.all():
			await ctx.respond(embed=EMBED_ERROR_QUEUE_EMPTY)
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Remove",
				description=f"Removed {song} from the queue.",
				color=discord.Color.green(),
			)
		)
	...
	@discord.option(
		"index", int, description="The index of the song to remove", required=True
	)
	async def remove_index(self, ctx: discord.ApplicationContext, index: int) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id)
		if index < 0 or index >= len(server.queue):
			await ctx.respond(
				embed=discord.Embed(
					title="Error",
					description=f"Index {index} out of range.",
					color=discord.Color.dark_red(),
				)
			)
			return
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Remove",
				description=f"Removed {song.name} from the queue.",
				color=discord.Color.green(),
			)
		)
	...
	async def clear(self, ctx: discord.ApplicationContext) -> None:
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Clear", description="Queue cleared.", color=discord.Color.green()
			)
		)

	@commands.slash_command(name="back", description="Goes back to the previous song")
	async def back(self, ctx: discord.ApplicationContext) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id)
		loop_song = server.loop_song
		loop_queue = server.loop_queue
	...
	random_command = SlashCommandGroup(
		name="random", description="Commands related to random mode"
	)
	...
	async def random_toggle(self, ctx: discord.ApplicationContext, state: bool) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id)
		if state is None:
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Random",
				description=f"Random mode set to {'on' if state else 'off'}.",
				color=discord.Color.green(),
			)
		)
	...
	async def random(self, ctx: discord.ApplicationContext) -> None:
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Random",
				description=f"Random mode is {'on' if server.random else 'off'}.",
				color=discord.Color.green(),
			)
		)

	play = SlashCommandGroup(
		name="play-queue",
		description="Commands related to playing songs from the queue",
	)
	...
	async def play_queue_song(self, ctx: discord.ApplicationContext, song: str) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id).prefetch_related(
			"queue", "queue__song"
		)
		...
		index = get_index_from_title(
			song, [queue_elem.song for queue_elem in server.queue]
		)
		if index == -1:
			await ctx.respond(
				embed=discord.Embed(
					title="Error",
					description=f"Song {song} not found in the queue.",
					color=discord.Color.dark_red(),
				)
			)
			return
		...
		await ctx.respond(
			embed=discord.Embed(
				title="Play",
				description=f"Playing [{song}]({server.queue[index].song.url}).",
				color=discord.Color.green(),
			)
		)
	...
	@discord.option(
		"index", int, description="The index of the song to play", required=True
	)
	async def play_queue_index(
		self, ctx: discord.ApplicationContext, index: int
	) -> None:
		await ctx.response.defer()
		server = await Server.get(server_id=ctx.guild.id).prefetch_related(
			"queue", "queue__song"
